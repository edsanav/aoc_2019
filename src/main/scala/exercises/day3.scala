...
object day3 extends App {
  ...
  case class Horizontal(y:Int, x1:Int, x2:Int) extends Line{
    ...
    def crossPoint(l:Horizontal):Option[Point] = None
    def crossPoint(l:Vertical):Option[Point] = {
      if (x1 <= l.x  &&  l.x <= x2) Some(Point(l.x, y))
      else None
    }
  ...
  case class Vertical(x:Int, y1:Int, y2:Int) extends  Line {
    ...
    def crossPoint(l:Vertical):Option[Point] = None
    def crossPoint(l:Horizontal):Option[Point] = {
      if (y1 <= l.y  &&  l.y <= y2) Some(Point(x, l.y))
      else None
    }
  }
